...
function Booking() {
  ...
  const [availableTimeSlots, setAvailableTimeSlots] = useState([]); // Доступные временные слоты с бэкенда
  ...
  // Генерируем 30-минутные слоты (9:00 - 18:00 по умолчанию, или из данных доктора)
  const generate30MinuteSlots = (startTime = "09:00", endTime = "18:00") => {
    const slots = [];
    ...
    const fetchAvailableSlots = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Endpoint doesn't exist yet - use generated slots as fallback
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedDate < todayDate) {
        setError("O'tgan sanani tanlash mumkin emas");
        setIsSubmitting(false);
        return;
      }

      // Validate time is not in the past (if date is today)
      if (selectedDate.getTime() === todayDate.getTime() && formData.time) {
        const now = new Date();
        ...
        if (slotTimeMinutes <= currentTimeMinutes) {
          setError("O'tgan vaqtni tanlash mumkin emas");
          setIsSubmitting(false);
          return;
        }
      }
